...
router.get("/last-update", async (req, res) => {
  ...
  // try {
  connection.query(
    `SELECT date FROM terminationrecords ORDER BY id DESC LIMIT 1;`,
...
function getMembersShouldTerminate(res, datesToTerminate) {
  ...
  connection.query(
    `SELECT memberID, membershipNo, nameWinitials, status, lastMembershipPaid FROM members WHERE status != 'Terminated';`,
    ...
    async function (error, results, fields) {
      ...
      connection.query(
        `INSERT INTO terminationrecords (date) VALUES ('${today.toISOString()}') `,
      ...
      if (outdatedCount === 0)
        return res.status(200).json({ msg: `0 members terminated` });
...
function setTerminated(memberID, name, memNo, outdatedCount, res) {
  ...
  connection.query(
    `UPDATE members
    SET status='Terminated', dot='${dot}' WHERE memberID='${memberID}';`,
    (error, results, fields) => {
      if (error) throw error;
      ...
      if (i == outdatedCount) {
        res.status(200).json({
          list: terminatedMembers,
          msg: `Successfully Terminated ${success} Members`,
        });
      }
    }
  );
